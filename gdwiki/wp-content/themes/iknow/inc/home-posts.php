...
function iknow_get_home_posts() {
	...
	foreach ( $categories as $cat ) {
		...
		// [!] MODIFIED
		$out .= '<div class="column home-column is-full-tablet is-half-desktop is-one-third-widescreen">';
		$out .= '<div class="panel home-panel ' . esc_attr( $panel_color ) . '">';
...
function iknow_home_panel_tabs( $cat_ID ) {
	...
	foreach ( $elements as $key => $val ) {
		if ( $i === 0 ) {
			$header .= '<a href="#' . $key . '-' . $cat_ID . '" class="is-active" data-tab="' . esc_attr( $key ) . '"' . iknow_panel_toogle( $elements, $key, $cat_ID ) . '>' . esc_html( $val ) . '</a>';
		} else {
			$header .= '<a href="#' . $key . '-' . $cat_ID . '" class="" data-tab="' . esc_attr( $key ) . '"' . iknow_panel_toogle( $elements, $key, $cat_ID ) . '>' . esc_html( $val ) . ' </a>';
		}
		$i ++;
	}
	$header .= '</p>';
