            ...
            <div class="col">
				...
				if ( ! empty( $_POST['action'] ) && ! empty( $_POST['wow_support_field'] ) ) {
					...
					if ( wp_verify_nonce( $_POST['wow_support_field'], 'wow_support_action' )
						...
						if ( empty( $name ) ) {
							$error[] = esc_attr__( 'Please, Enter your Name.', 'herd-effects' );
						}
						if ( empty( $email ) ) {
							$error[] = esc_attr__( 'Please, Enter your Email.', 'herd-effects' );
						}
						if ( empty( $subject ) ) {
							$error[] = esc_attr__( 'Please, Enter Subject of Message.', 'herd-effects' );
						}
						if ( empty( $message ) ) {
							$error[] = esc_attr__( 'Please, Enter your Message.', 'herd-effects' );
						}
						...
						if ( count( $error ) == 0 ) {
							...
							echo '<div class="notice notice-success is-dismissible"><p>'
							     . esc_attr__( 'Your Message sent to the Support.', 'herd-effects' ) . '</p></div>';
						}
					} else {
                            ...
                            <div class="field">
                                <p class="control is-expanded has-icons-left">
                                    <input class="input is-radiusless is-dark" type="email" name="email"
                                           placeholder="Email"
                                           required value="<?php echo sanitize_email(get_option( 'admin_email' )); ?>">
                                    <span class="icon is-small is-left">
                            ...
                            <div class="field has-addons">
                                <div class="control">
									...
									<select name="type" class="is-radiusless">
										<option value="Issue"><?php esc_html_e( 'Issue', 'herd-effects' ); ?></option>
										<option value="Idea"><?php esc_html_e( 'Idea', 'herd-effects' ); ?></option>
									</select>
									</span>
                                </div>
                    ...
                    <div class="field is-horizontal">
                        ...
                        <div class="field-body">
                            <div class="field">
                                <div class="control">
	                                <?php
	                                $content   = esc_attr__( 'Enter Your Message', 'herd-effects' );
