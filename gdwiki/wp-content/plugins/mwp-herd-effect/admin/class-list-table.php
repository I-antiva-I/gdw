...
class Wow_List_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$actions       = array(
			'edit'      => '<a href="' . esc_url( $edit_url ) . '">' . esc_attr__( 'Edit', 'herd-effects' ) . '</a>',
			'duplicate' => '<a href="' . esc_url( $duplicate_url ) . '" style="color:green;">' . esc_attr__( 'Duplicate', 'herd-effects' )
			               . '</a>',
			'delete'    => '<a href="' . esc_url( wp_nonce_url( $delete_url, $slug . '_nonce' ) ) . '" class="has-text-danger">' . esc_attr__( 'Delete', 'herd-effects' ) . '</a>',
		);
	...
	private function table_data() {
		...
		if ( $result ) {
			foreach ( $result as $key => $value ) {
				...
				$data[] = array(
					'ID'    => $value->id,
					'title' => '<a href="admin.php?page=' . $slug . '&tab=settings&act=update&id=' . $value->id
					           . '">' . $title . '</a>',
