...
if ( $info === 'saved' ) {
	$message_data = esc_attr__( 'Item Added', 'herd-effects' );
	echo '<div class="updated" id="message"><p><strong>' . $message_data . '</strong>.</p></div>';
} elseif ( $info === 'update' ) {
	$message_data = esc_attr__( 'Item Updated', 'herd-effects' );
	echo '<div class="updated" id="message"><p><strong>' . $message_data . '</strong>.</p></div>';
} elseif ( $info === 'delete' ) {
	...
	if ( ! empty( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], $this->plugin['slug'] . '_nonce' ) ) {
		...
		if ( absint( $delete ) > 0 ) {
			echo '<div class="updated" id="message"><p><strong>' . esc_html__( 'Item Deleted', 'modal-window' ) . '</strong>.</p></div>';
		}
	}
...
$tabs_arr = array(
	'list'      => esc_attr__( 'List', 'herd-effects' ),
	'settings'  => esc_attr__( 'Add new', 'herd-effects' ),
	'extension' => esc_attr__( 'Pro Features', 'herd-effects' ),
	'support'   => esc_attr__( 'Support', 'herd-effects' ),
	'tools'     => esc_attr__( 'Import/Export', 'herd-effects' ),
	'rate'      => esc_attr__( 'Rate', 'herd-effects' ),
);
...
?>
    <div class="wrap">
        ...
        <a href="?page=<?php echo $this->plugin['slug']; ?>&tab=settings" class="page-title-action">
			<?php esc_attr_e( 'Add New', 'herd-effects' ); ?></a>
			...
			if ( $tab === 'settings' ) {
				...
				if ( ! empty( $action ) && $action === 'update' ) {
					echo '<a class="nav-tab' . esc_attr( $class ) . '" href="?page=' . esc_attr($this->plugin['slug']) . '&tab='
					     . esc_attr( $tab ) . '">' . esc_attr__( 'Update', 'herd-effects' ) . ' #'
				...
				echo '<a class="nav-tab' . esc_attr( $class ) . '" href="' . esc_url( $rating ) . '" target="_blank"><span class="dashicons dashicons-star-filled" style="color:#ffcc01;"></span> ' . esc_attr( $name ) . '</a>';
