    ...
    <div class="about-wrap wow-box">
        <div class="feature-section one-col">
            <div class="col">
				...
				<?php $error = array();
				if ( ! empty( $_POST['action'] ) && ! empty( $_POST['wp_support_field'] ) ) {

					if ( wp_verify_nonce( $_POST['wp_support_field'], 'wp_support_action' )
						...
						if ( empty( $name ) ) {
							$error[] = esc_attr__( 'Please, Enter your Name.', 'counter_box' );
						}
						if ( empty( $email ) ) {
							$error[] = esc_attr__( 'Please, Enter your Email.', 'counter_box' );
						}
						if ( empty( $subject ) ) {
							$error[] = esc_attr__( 'Please, Enter Subject of Message.', 'counter_box' );
						}
						if ( empty( $message ) ) {
							$error[] = esc_attr__( 'Please, Enter your Message.', 'counter_box' );
						}
						...
						if ( count( $error ) == 0 ) {
							...
							wp_mail( 'yoda@wow-company.com', $subject, $message, $headers );
							echo '<div class="notice notice-success is-dismissible"><p>'
							     . esc_attr__( 'Your Message sent to the Support.', 'counter_box' ) . '</p></div>';
						}
					} else {
                ...
                <form method="post" action="" class="wow-plugin">
                            ...
                            <div class="field has-addons">
                                <div class="control">
									...
									<select name="type" class="is-radiusless">
										<option value="Issue"><?php esc_attr_e( 'Issue', 'counter_box' ); ?></option>
										<option value="Idea"><?php esc_attr_e( 'Idea', 'counter_box' ); ?></option>
									</select>
									</span>
                                </div>
					...
					<?php wp_nonce_field( 'wp_support_action', 'wp_support_field' ); ?>
